...
{
  QGauss<dim>     quadrature_formula(mapping_degree);
  QGauss<dim - 1> face_quadrature_formula(mapping_degree);
...
template <int dim>
void
Poisson<dim>::residual_error_estimator()
{
  QGauss<dim> quad{mapping_degree};

  std::map<types::boundary_id, const Function<dim> *> neumann_bc;
  for (const auto id : neumann_ids)
    neumann_bc[id] = &neumann_boundary_condition;
  QGauss<dim - 1> face_quad(mapping_degree);
  ...
  FEValues<dim> fe_values(
    *fe, quad, update_hessians | update_JxW_values | update_quadrature_points);
...
Poisson<dim>::compute_error()
{
  ...
  error_table.add_extra_column(
    "estimator", [global_estimator]() { return global_estimator; }, true);
